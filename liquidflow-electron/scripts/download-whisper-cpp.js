...
async function main() {
  ...
  if (args.isCurrent) {
    // whisper.cpp ships no native Windows-on-ARM build, so target the x64 binary
    // on any Windows host/target; it runs under emulation on ARM. Parakeet is the
    // native ARM64 engine — whisper is the emulated alternative here.
    let platformArch = args.platformArch;
    if (args.targetPlatform === "win32" && platformArch !== "win32-x64") {
      console.log(`No native whisper build for ${platformArch}; using win32-x64 (emulated on ARM).`);
      platformArch = "win32-x64";
    }

    if (!BINARIES[platformArch]) {
