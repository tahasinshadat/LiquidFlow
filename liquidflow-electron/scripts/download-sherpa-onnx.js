...
const BINARIES = {
  ...
  // Native Windows-on-ARM build. sherpa-onnx doesn't publish a plain
  // "-win-arm64-shared" alias, but it does ship per-config CI artifacts; the
  // static-CRT Release ("MT-Release") variant is self-contained (no ARM64 VC++
  // redist needed) and bundles onnxruntime.dll + the websocket/diarization exes.
  // A native arm64 sherpa child process runs at full speed even when the Electron
  // shell is x64-emulated, so this is what gives Parakeet its native-app speed.
  "win32-arm64": {
    archiveName: `sherpa-onnx-v${SHERPA_ONNX_VERSION}-win-arm64-shared-MT-Release.tar.bz2`,
    binaryPath: "sherpa-onnx-offline-websocket-server.exe",
    outputName: "sherpa-onnx-ws-win32-arm64.exe",
    diarizeBinaryPath: "sherpa-onnx-offline-speaker-diarization.exe",
    diarizeOutputName: "sherpa-onnx-diarize-win32-arm64.exe",
    libPattern: "*.dll",
  },
  "linux-x64": {
...
async function main() {
  ...
  if (args.isCurrent) {
    ...
    // Windows-on-ARM is a first-class target: always ship the native arm64 sherpa
    // build alongside the current-arch one so Parakeet runs natively on ARM even
    // when this build ran on an x64 host. Best-effort (non-fatal).
    if (args.targetPlatform === "win32" && args.platformArch !== "win32-arm64") {
      console.log("Also ensuring native win32-arm64 sherpa build (for Windows-on-ARM):");
      const armOk = await downloadBinary("win32-arm64", BINARIES["win32-arm64"], args.isForce);
      if (!armOk) {
        console.warn("  win32-arm64: could not fetch (non-fatal; x64 fallback still present)");
      }
    }
    ...
    if (args.shouldCleanup) {
      // Keep both the current-arch binaries and the native win32-arm64 ones we ship
      // for Windows-on-ARM.
      const keepArchs = new Set([args.platformArch]);
      if (args.targetPlatform === "win32") keepArchs.add("win32-arm64");
      const keepPrefixes = [];
      for (const a of keepArchs) {
        keepPrefixes.push(`sherpa-onnx-ws-${a}`, `sherpa-onnx-diarize-${a}`);
      }
      const files = fs.readdirSync(BIN_DIR).filter((f) => f.startsWith("sherpa-onnx"));
      files.forEach((file) => {
        if (!keepPrefixes.some((p) => file.startsWith(p))) {
